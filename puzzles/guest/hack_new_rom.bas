...
CDXC     i = I
CDXCV    DIM n AS INTEGER
D        DIM m AS INTEGER
DV       i = i + I
DX       IF i > pwdcount+I THEN GOTO DLX
DXV      n = XLVII
DXX      n = n + I
DXXV     IF n > LVII THEN GOTO DV
DXXX     m = XLVIII
DXXXV    REM PRINT "check " + words(i-1) + CHR(n) + CHR(m) + "..." + CHR(10) 
DXL      IF CHECKPASS(username, words(i-I) + CHR(n) + CHR(m)) THEN GOTO DLXX
DXLV     m = m + I
DL       IF m > LVII THEN GOTO DXX
DLV      GOTO DXXXV
DLX      PRINT "no smart mathes for user " + username + CHR(X)
DLXV     END
DLXX     PRINT "found match! for user " + username + CHR(X)
DLXXV    PRINT "password: " + words(i-I) + CHR(n) + CHR(m) + CHR(X)
DLXXX    END
